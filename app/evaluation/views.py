...
def start(request):
	...
	if getXls:
		...
		for obj in aAntwortenM:
			...
			ws.write(row_num, 2, obj.beispiel, font_style)
			ws.write(row_num, 3, obj.wiedergaben, font_style)
			ws.write(row_num, 4, str(obj.gewOrt), font_style)
			ws.write(row_num, 5, obj.hochdeutsch, font_style)
			ws.write(row_num, 6, obj.orf, font_style)
			ws.write(row_num, 7, obj.pOrt, font_style)
			ws.write(row_num, 8, obj.aufgefallen, font_style)
			ws.write(row_num, 9, obj.zeit, font_style_datetime)
			ws.write(row_num, 10, obj.spiel.pk, font_style)
			ws.write(row_num, 11, obj.spiel.zeit, font_style_datetime)
			ws.write(row_num, 12, obj.audiodatei.pk, font_style)
			ws.write(row_num, 13, obj.audiodatei.file, font_style)
			ws.write(row_num, 14, str(obj.audiodatei.ort), font_style)
			ws.write(row_num, 15, obj.audiodatei.benutzt, font_style)
			ws.write(row_num, 16, obj.spiel.spieler.pk, font_style)
			ws.write(row_num, 17, obj.spiel.spieler.zeit, font_style_datetime)
			ws.write(row_num, 18, obj.spiel.spieler.geburtsjahr, font_style)
			ws.write(row_num, 19, obj.spiel.spieler.gesch, font_style)
			ws.write(row_num, 20, obj.spiel.spieler.taetigkeit, font_style)
			ws.write(row_num, 21, str(obj.spiel.spieler.wohnort), font_style)
			ws.write(row_num, 22, str(obj.spiel.spieler.wohnortLeben), font_style)
			ws.write(row_num, 23, obj.spiel.spieler.wohnortEltern, font_style)
			ws.write(row_num, 24, obj.spiel.spieler.erkannt, font_style)
			ws.write(row_num, 25, obj.spiel.spieler.anmerkungen, font_style)
		wb.save(response)
		return response
	else:
		if aSeite > 0:
